...
module.exports = function(params) {
 ...
 this.initiate = function() {//reading and initiate page data from json file
	...
	if($type == 'page') {
		if(fs.existsSync($views_dir + '/assets/scripts/' + $name + '.js'))
			$vars['inline_scripts'] = fs.readFileSync($views_dir + '/assets/scripts/' + $name + '.js' , 'utf-8')
	  ...
	  if($vars[which+'s']) {//if we have $vars["scripts"] or $vars["styles"]
		...
		if($vars['ie_'+which+'s']) {
			...
			for(var i in ie_scripts) if(map_json[ie_scripts[i].name])
				ie_mapped_scripts.push({"version" : ie_scripts[i].version , "path" : map_json[page_scripts[i].name]})
